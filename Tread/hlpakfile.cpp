...
void* CHLPakFile::GetFileObject(CTexture* pTex, int nContext)
{ return &m_hFile; }

// Seeks to the file's position.
bool CHLPakFile::SeekToFile(CTexture* pTex)
{
...
int CHLPakFile::GetFileOffset(CTexture* pTex)
{
	if(pTex->m_nHandle < 0 || pTex->m_nHandle >= m_nNumFiles)
		return -1;
...
bool CHLPakFile::ClaimFile(const char* szFilename)
{
	CFile hFile;
	...
	hFile.Close();
	return nFileType == HLPAKID;
}

// Get's the list of texture's in the pak.
CTexture** CHLPakFile::GetTextureList(int& nNumTextures) const
{ nNumTextures = m_nNumTexs; return m_pList; }

bool CHLPakFile::Close()
{
	m_hFile.Close();
	m_nNumFiles = 0;
	if(m_pDir != NULL)
...
bool CHLPakFile::OpenAndInitialize(const char* szFileName)
{
	...
	m_pDir = new HLFILEENTRY[nDirCount];
	if(m_pDir == NULL)
		return false;
